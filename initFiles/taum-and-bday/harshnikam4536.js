...
function taumBday(b, w, bc, wc, z) {
    // Find the minimum cost for each gift
    const effectiveBc = Math.min(bc, wc + z);
    const effectiveWc = Math.min(wc, bc + z);
    
    // Use BigInt to handle very large numbers
    return BigInt(b) * BigInt(effectiveBc) + BigInt(w) * BigInt(effectiveWc);
}
